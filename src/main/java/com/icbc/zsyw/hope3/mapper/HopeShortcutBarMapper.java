...
public interface HopeShortcutBarMapper {
  ...
  /*  @Select("SELECT hs.shortcutbarname,hs.shortcut_image AS shortcutImage,hs.shortcutbardescript FROM hopeshortcutbar hs INNER JOIN \n" +
   ...
   /* @Select("SELECT hm.moduleid,hm.modulename,hm.shortname,hm.description,hm.icon,hm.image,hm.modulegroupname,hm.url FROM hopemodule hm INNER JOIN\n" +
    ...
    List<HopeShortcutBar> queryShortcutBarByAamid(String aamid);

    List<HopeShortcutBar> queryShortcutBarByDeptid(String deptid);

    List<HopeShortcutBar> queryShortcutBarByOdeptid(String odeptid);

    List<HopeShortcutBar> queryShortcutBarByPrivtype();

    List<HopeShortcutBar> queryByadoid(String aamid, String deptid, String odeptid);
}